...
def main(params, drop=True):
    ...
    extract = Extract(params.file_path)

    # Transform the data into a pandas dataframe
    logging.info("Transforming source file to pandas.DataFrame...")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t",
        "--table_name",
        help="Destination database table where to load the data.",
        required=True,
    )
    parser.add_argument(
        "-U", "--url", help="Source URL to download data from.", required=False
    )
    parser.add_argument("-f", "--file_path", help="File to ingest.", required=True)
    parser.add_argument(
        "-d",
        "--dates",
